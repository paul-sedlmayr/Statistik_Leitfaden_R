...
```{r, echo=FALSE}
knitr::opts_chunk$set(tidy = TRUE, fig.width = 7, fig.height = 4, dev = "png",
...
# Diese Funktion müssen wir für alle noch nicht heruntergeladenen Packages anwenden. 

# Packages laden
library(tidyverse) # All-around package
#library(dplyr) # Package  aus dem Tidyverse
...
library(ggeffects) # Modell standardisieren
library(performance) # Convenience Functions wie check_model()
...
```{r, label=fig-histbaser}
hist(df$IQ, col = "#FB8C00")
```

### Violin Plots

```{r, label=fig-violin, fig.width=10, fig.height=4}
library(rempsyc)
library(cowplot)

m1 <- nice_violin(df, "IQ", xlabels = NULL, alpha = 0.6, colours = c("#FB8C00"))
m2 <- nice_violin(df, "correct_items", obs = "jitter", xlabels = NULL, alpha = 0.6, colours = c("#8E24AA"))
m3 <- nice_violin(df, "BDI", obs = "jitter", alpha = 0.8,  colours = c("#00ACC1")) 
plot_grid(m1, m2, m3, ncol = 3)
```
...
: Interpretation von Cohen’s $d$ nach Konventionen (Cohen, 1988). Die Bewertung der Effektgröße sollte jedoch kontextabhängig erfolgen, unter Berücksichtigung des Forschungsfelds und bisheriger Befunde. {#tbl-cohen_d}
...
Am besten plotten wir die Daten zusätzlich:

```{r, message = FALSE, warning = FALSE}
library(patchwork)   # stacking panels
#library(ggdist)

GROUP_COLORS <- c("#CC79A7", "#009E73", "#4A90A4")
...
#  Arguments:
#    data       : a data frame
#    group_var  : unquoted column name of the grouping variable (factor/character)
#    dep_var    : unquoted column name of the dependent variable (numeric)
#    x_label    : label for the shared x-axis (default: variable name)
#    colors     : character vector of 2 hex colors for the two groups
#    alpha_fill : transparency for density fills (0–1, default 0.55)
#    bw         : bandwidth method for density ("nrd0", "SJ", numeric, …)
#
#  Returns: a patchwork ggplot object (print it or save with ggsave)
# ============================================================

plot_density_box <- function(data,
                             group_var,
                             dep_var,
                             x_label    = NULL,
                             colors     = GROUP_COLORS,
                             ...
                             bw         = "nrd0",
                             show_legend   = TRUE,
                             legend_title  = NULL) {   # ← add this{

  # --- tidy evaluation: allow bare column names ---------------
  gv  <- rlang::ensym(group_var)
  dv  <- rlang::ensym(dep_var)

  df <- data %>%
    select(group = !!gv, value = !!dv) %>%
  ...
  if (n_groups != 2)
    warning("Function designed for exactly 2 groups; ",
            n_groups, " detected — proceeding anyway.")

  # colour mapping
  col_map <- setNames(colors[seq_len(n_groups)], groups)
  ...
  x_lab <- if (is.null(x_label)) rlang::as_label(dv) else x_label


  # ── PANEL A: overlapping density curves ───────────────────
  p_density <- ggplot(df, aes(x = value, fill = group, colour = group)) +
    geom_density(alpha = alpha_fill, bw = bw, linewidth = 0.8) +
    scale_fill_manual(values = col_map,   name = legend_title) +
    scale_colour_manual(values = col_map, name = legend_title) +
  ...
  p_box <- ggplot(df, aes(x = value, y = group,
                           fill = group, colour = group)) +

    geom_jitter(height = 0.15, size = 1.4, alpha = 0.55, shape = 16) +

    geom_boxplot(width = 0.45, alpha = 0.15,
  ...
  combined <- p_density / p_box +
    plot_layout(heights = c(1.6, 1))   # density taller than boxes

  return(combined)
}
```

```{r fig-densityplot, fig.width=7, fig.height=4}
plot_density_box(
    data      = df_2_long,
    group_var = time,
    dep_var   = wellbeing,
    x_label   = "Well Being",
    legend_title = "Zeitpunkt"
  )
```
...
```{r}
library(ggeffects)
ggemmeans(anova_2_bet, terms = c("therapy_group", "gender"))
ggemmeans(anova_2_bet, terms = c("gender", "therapy_group")) %>% plot
```
...
```{r label=fig-interactionanova3, fig.width=7, fig.height=4}
...
## Effektgröße Eta-Quadrat $\hat \eta^2$
...
: Interpretation von Eta-Quadrat nach Konventionen (Cohen, 1988). Die Bewertung der Effektgröße sollte jedoch kontextabhängig erfolgen, unter Berücksichtigung des Forschungsfelds und bisheriger Befunde. {#tbl-eta}
...
```{r eta_sq}
library(effectsize)
eta_squared(anova_mixed, alternative = "two.sided")
```

```{r}
effectsize::eta_squared(anova_mixed, alternative = "two.sided") %>% plot()
```
...
```{r}
omega_squared(anova_mixed, alternative = "two.sided")
```
...
plot(estimated_marginal_means)
estimated_marginal_means
```

Nun untersuchen wir den geplanten Kontrast und plotten den Kontrast inklusive Konfidenzintervall.

```{r, label=fig-contrastci}
...
```{r, label=fig-contrastplot}
library(rempsyc)

nice_table(nice_contrasts(data = df_2_wide, response = "wellbeing_t2", group = "therapy_group", effect.type = "cohens.d", bootstraps = 1000))
```
...
emm_bet <- emmeans(anova_bet, ~ therapy_group) 

contrast_matrix <- contrast(emm_bet,
...
: Interpretation der Pearson-Korrelation $r$ nach Konventionen (Cohen, 1988). Die Bewertung der Effektgröße sollte jedoch auch kontextabhängig erfolgen, unter Berücksichtigung des Forschungsfelds und bisheriger Befunde. {#tbl-Cohen_r}
...
Die standardisierten Regressionskoeffizienten, also der $\beta$-Koeffizient, werden leider nicht automatisch mit ausgegeben. Mit dem Package `lm.beta` können wir diese einfach bekommen. In der einfachen linearen Regression ist der standardisierte Regressionskoeffizient $\beta$ identisch mit der Pearson-Korrelation. Die ist in der MLR nicht der Fall, es sei denn die Prädiktoren haben keine gemeinsame Varianz.
...
Eine anderer Weg ist `standardize_parameters()` aus dem `effectsize`-Package.

```{r}
library(effectsize)
standardize_parameters(model_simple_regression)

standardize_parameters(model_simple_regression) %>% plot()
```
...
Hinweis: wenn wir einen Interaktionsterm von zwei Prädiktoren ($x_1 \cdot x_2$) sowie die einzelnen Prädiktoren ($x_1$ und $x_2$) im Modell haben, sollten wir die Prädiktoren zurvor zentrieren. Das bedeutet, wir ziehen von jedem Wert den Mittelwert ab, ähnlich wie im Rahmen der z-Standardisierung. Das hat den folgenden Grund: wenn $x_1$ als einfacher Prädiktor auch im Interaktionsterm ($x_1 \cdot x_2$) vorkommt, sind der Prädiktor und der Interaktionsterm korreliert. Es könnte also zur Multikollinearität kommen, was die Schätzungen der Regressionsgewichte ($\beta_1$) und deren Standardfehler verzerrt. Im Folgenden erstellen wir neue Variablen: die zentrierten Prädiktoren und nutzen diese im Modell.

```{r}
df$numerical_ability_cent <- df$numerical_ability - mean(df$numerical_ability, na.rm = TRUE)
df$WMS4_delayed_recall_cent <- df$WMS4_delayed_recall - mean(df$WMS4_delayed_recall, na.rm = TRUE)

model_mlr <- lm(IQ ~ numerical_ability_cent + WMS4_delayed_recall_cent + numerical_ability_cent * WMS4_delayed_recall_cent, data = df)
...
Für das Konfidenzintervall zum standardisierten Regressionsgewicht:

```{r, label=mlrciplot, fig.width=8, fig.height=6}
library(effectsize)
standardize_parameters(model_mlr)

standardize_parameters(model_mlr) %>% plot()
```
...
Der folgende Plot eignet sich, um den Interaktionsterm genauer zu untersuchen.

```{r, label=mlreffectsplot, fig.width=8, fig.height=6}
library(ggeffects)
ggeffect(model_mlr, terms = c("WMS4_delayed_recall_cent", "numerical_ability_cent")) %>% plot()
```
...
##### Alle Plots auf einen Blick

R hat eine sehr simple Funktion, Plots zu den Voraussetzungen zu erstellen. Wir wenden einfach `plot()` am zuvor erstellen Regressions-Objekt an:

```{r label=fig-regression_plots, fig.width=8, fig.height=6}
model_mlr <- lm(IQ ~ numerical_ability_cent + WMS4_delayed_recall_cent + numerical_ability_cent * WMS4_delayed_recall_cent, data = df)

plot(model_mlr)
```

Das Package `performance()` hat eine ähnliche Funktion `check_model()` für schöne Voraussetzungs-Plots für alle linearen Modelle.

```{r label=fig-regression_assumptions, fig.width=12, fig.height=8}
library(performance)
check_model(model_mlr)
```
...
```{r label=fig-ggpairscatterplot2, message=FALSE, warning=FALSE}
library(GGally)
ggpairs(df, columns = c("IQ", "numerical_ability_cent", "WMS4_delayed_recall_cent"),
        lower = list(continuous = wrap("smooth", method = "loess", se = TRUE)),
...
In Regressionsmodellen eignet sich die Cooks-Distance besonders gut, um die Auswirkung eines Ausreißers auf das Ergebnis zu untersuchen. Im Plot "Influential Observations" in @fig-regression_assumptions ist ersichtlich, falls einzelne Datenpunkte einen zu hohen Einflussh haben.

```{r}
library(performance)
check_outliers(model_mlr)
```
...
::: callout-warning
### Multikollinearität bei Multipler Linearer Regression mit Interaktionsterm

Wenn ein Prädiktor in der MLR einfach und in einem Interaktionsterm in der Modellgleichung ist, ist dieser Prädiktor sehr anfällig für Multikollinearität, da derselbe Prädiktor zwei mal im Modell vorkommt. Das führt zu fehlerbehafteten Schätzungen für den Prädiktor. Der Prädiktor kann einfach zentriert werden, um Multikollinearität zu vermeiden.
:::
...
## Moderierte Regression mit diskretem Prädiktor
...
```{r}
df$IQ_cent <- df$IQ - mean(df$IQ, na.rm = TRUE)
```
...
```{r label=fig-dummyscatter2}
library(ggeffects)
ggeffect(model_dummy_regression, terms = c("IQ_cent", "experimentalgruppe")) %>% plot()
```

```{r label=fig-dummyscatter, warning=FALSE, fig.width=7, fig.height=5}
...
Auch hier müssen wir unseren Prädiktor, welcher sowohl einfach als auch quadratisch vorkommt, zentieren.

```{r}
df$confidence_cent <- df$confidence - mean(df$confidence, na.rm = TRUE)
```
