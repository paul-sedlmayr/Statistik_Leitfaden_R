...
```{r}
data <- data %>% mutate(BDI_z2 = ((BDI - mean(BDI)) / sd(BDI)))
```
...
```{r}
data$BDI_c <- data$BDI - mean(data$BDI)
```
...
data_3_long <- pivot_longer(data = data_3, cols = c("wellbeing_t0", "wellbeing_t1", "wellbeing_t2"), names_to = "time", values_to = "wellbeing")
...
```{r}
data_3_wide <- pivot_wider(data = data_3_long, names_from = "time", values_from = "wellbeing", id_cols = "participant_id")
...
data_wor$participant <- c(1:length(data_wor$Wor)) 
```
...
```{r}
anova_result <- aov_ez(id = "participant", dv = "Wor", between = "S_E", data = data_wor, anova_table = list(es = "pes"))

anova_result

# Alternativ mit aov()
#anova_result <- aov(Wor ~ S_E, data = data_wor)
```
...
```{r}
anova_result <- aov_ez(id = "participant_id", dv = "wellbeing", within = "time", anova_table = list(es = "pes"), data = data_3_long)
anova_result
```

```{r}
ggplot(data_3_long, aes(x = time, y = wellbeing, fill = time)) +
...
```{r}
anova_result <- aov_ez(id = "participant", dv = "Wor", between = c("gender", "S_E"), data = data_wor, anova_table = list(es = "pes"))
anova_result
...
interaction.plot(data_wor$S_E, data_wor$gender, data_wor$Wor, col = c("blue", "red"), lty = 1, lwd = 2, legend = TRUE,
...
Wir könnten zum Beispiel die a-priori Hypothese aufgestellt haben, dass sich Gruppe 1 und Gruppe 2 unterscheiden. Hierfür müssen wir die Mittelwerte mit Kontrastgewichten versehen. Wir gewichten den Mittelwert von Gruppe 1 mit +1, von Gruppe 2 mit -1. Nachdem uns Gruppe 3 hier nicht interessiert, wird deren Mittelwert mit 0 gewichtet. Wir können mit `contrast()` die Kontraste eines Faktors manuell überschreiben.
...
```{r}
set.seed(100)
nice_contrasts(response = "Wor", group = "S_E", data = data_wor, effect.type = "cohens.d", bootstraps = 1000)
```
...
contrasts(data_wor$S_E)

anova_bet <- aov(Wor ~ S_E, data = data_wor)
...
Ausreißer können das Ergebnis der ANOVA stark beeinflussen. Wir können den Residual Plot (@fig-residualplotanova) nutzen, um Ausreißer zu identifizieren.
...
data <- data %>%
  mutate(Rauchen_dummy = case_when(
    Rauchen == "Ja" ~ 1,
    Rauchen == "Nein" ~ 0,
...
```{r}
model_dummy_regression <- lm(Extraversion ~ Rauchen_dummy + Neurotizismus + Rauchen_dummy * Neurotizismus , data = data)
...
Um die Normalverteilung der Residuen zu untersuchen, können wir wieder ein Histogram und einen QQ-Plot der standardisierten Residuen erstellen. Der Code ist wieder identisch zum QQ-Plot in der ANOVA (@fig-qqplotanova).
...
ggplot(data, aes(x = Alter, y = Offenheit)) +
